...
class AppPage(tk.Frame):
	...
	def clicked_item_in_Listbox(self, event):
		if not self.update_mode:
			...
			self.table_info[3][1].configure(text=tgl_lhr)
			self.table_info[4][1].configure(text=ortu)
	...
	def create_detail_right_footer(self):
		...
		for feature in features:
			...
			button.grid(row=0, column=features.index(feature), sticky="nsew", padx=20, pady=20)
	...
	def clicked_update_btn(self):
		...
		for feature in features:
			...
			button.grid(row=0, column=index, sticky="nsew", padx=10, pady=20)
			self.buttons_features.append(button)
	...
	def clicked_add_new_btn(self):
		...
		self.virt_img = tk.PhotoImage(width=1, height=1)
		self.title_header_label = tk.Label(self.detail_add_new_header, text="Tambah Murid Baru", font=("Arial", 30), width=frame_w, height=frame_h, image=self.virt_img, compound="c", bg="white")
		...
		features = ['Add', 'Cancel']
		commands = [self.clicked_save_add_new_btn, self.clicked_cancel_btns]
		self.buttons_features = []
		for feature in features:
			index = features.index(feature)
	...
	def clicked_save_btn(self):
		...
		if confirm:
			...
			tgl_lhr = self.entry_update_contact_vars[5].get()
			ortu = self.entry_update_contact_vars[6].get()
			...
			for contact in contacts:
				for phoneNumber, info in contact.items():
					if item_search in phoneNumber:
						...
						self.contacts_index.append(index_counter)
					elif item_search in info['address']:
						self.contacts_index.append(index_counter)
					elif item_search in info['email']:
						self.contacts_index.append(index_counter)
					elif item_search in info['tgl_lhr']:
						self.contacts_index.append(index_counter)
					elif item_search in info['ortu']:
						self.contacts_index.append(index_counter)
